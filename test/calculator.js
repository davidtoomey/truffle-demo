...
contract('Calculator', function(accounts) {
  ...
  it("should divide number in half", function() {
		
		var calculator;

		return Calculator.deployed().then(function(instance){
			calculator = instance;
			return calculator.half(20)
		}).then(function () {
			return calculator.getResult.call();
		}).then(function (result) {
			assert.equal(result.toNumber(), 10, "result after running half() should be 5");
		});
  });
  ...
  it("should double a number", function() {
  	...
  	return Calculator.deployed().then(function(instance) {
			calculator = instance;
			return calculator.double(10)
  	}).then(function() {
  		return calculator.getResult.call();
  	}).then(function (result) {
  		assert.equal(result.toNumber(), 20, "result after running double should be 20");
  	});
  });

  // Add Test Case#5 here
  it("should multiply by number", function() {
  	...
  	return Calculator.deployed().then(function(instance) {
  		...
  		assert.equal(result.toNumber(), 200, "result after running multiplyWithNumber should be 40");
  	});
  });

  // Add Test Case#6 here
  it("should divide by number", function() {
  	...
  	return Calculator.deployed().then(function(instance) {
  		...
  		assert.equal(result.toNumber(), 40, "result after running divideByNumber should be 40");
  	});
  });
